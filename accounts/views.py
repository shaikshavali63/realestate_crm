...
def login_view(request):
    ...
    try:
        ...
        facebook_enabled = SocialApp.objects.filter(provider="facebook").exists()
    except Exception:
        google_enabled = False
        facebook_enabled = False

    social_ctx = {
        "google_login_enabled": getattr(settings, "GOOGLE_LOGIN_ENABLED", False) and google_enabled,
        "facebook_login_enabled": getattr(settings, "FACEBOOK_LOGIN_ENABLED", False) and facebook_enabled,
    }
    ...
    return render(request, "accounts/login.html", social_ctx)
...
def agent_detail(request, agent_id):
    ...
    leads_qs = Lead.objects.select_related("property").order_by("-created_at")
    agent = (
        User.objects.filter(id=agent_id, is_staff=True, is_superuser=False)
        .select_related("profile")
        .prefetch_related(Prefetch("leads", queryset=leads_qs))
        .prefetch_related(Prefetch("property_set", queryset=Property.objects.order_by("title")))
        .annotate(total_leads=Count("leads"))
