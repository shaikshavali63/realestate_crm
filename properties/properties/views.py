...
def property_enquiry(request, pk):
    ...
    if not phone or not address:
        ...
        if not address:
            ...
            if addr_lead:
                address = addr_lead.address

    # Avoid duplicate leads for the same customer/property
    lead = Lead.objects.filter(email=request.user.email, property=property_obj).first()

    if lead is None:
        # Reuse the most recent lead created at registration (no property yet)
        lead = Lead.objects.filter(email=request.user.email, property__isnull=True).order_by('-created_at').first()
    ...
    if lead:
        ...
        lead.phone = phone or lead.phone or "Not Provided"
        lead.email = email
        lead.address = address or lead.address or "Not Provided"
        lead.source = "Website"
